...
namespace StarShop.ProductApi.Context;
...
public class AppDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder mb)
    {
       ...
       Property(p => p.Description).HasMaxLength(255).IsRequired();
        ...
        mb.Entity<Product>().
          Property(p => p.ImageUrl).HasMaxLength(255).IsRequired();

        mb.Entity<Product>().Property(p => p.Price).HasPrecision(12, 2);

        mb.Entity<Category>().HasMany(g => g.Products).WithOne(c => c.Category).IsRequired()
